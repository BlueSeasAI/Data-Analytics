...
def load_data():
    try:
        ...
        # Debug information
        st.write(f"Columns in dataset: {data.columns.tolist()}")
        ...
        if data['Date'].isna().any():
            st.warning("Some dates couldn't be converted properly")
        
        # More debug info if needed
        # st.write(f"Date column dtype: {data['Date'].dtype}")
        # st.write(f"First few dates: {data['Date'].head()}")
        
        return data
    except Exception as e:
        st.error(f"Error loading data: {e}")
        # Return an empty DataFrame with the expected columns to prevent further errors
        return pd.DataFrame(columns=['Date', 'Day', 'Revenue_Coffee', 'Revenue_Food', 
                                    'Revenue_Workspace', 'Revenue_Events', 'Customer_Count', 
                                    'New_Customers', 'Repeat_Customers', 'Avg_Ticket_Size',
                                    'Weather', 'Special_Promotion', 'Workshop_Topic'])
