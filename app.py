...
# Custom CSS styling with additional improvements
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
...
st.sidebar.image("Brew Minds logo.png", width=150)  # Replace with your actual logo
...
date_range = st.sidebar.date_input(
    ...
    min_value=min_date,
    max_value=max_date,
    format="dd/MM/yy"  # Correct Java SimpleDateFormat syntax
)
...
# Main page header with updated title
...
# ENHANCEMENT 5: Interactive "What-If" Scenario Simulator with Improved Layout
col1, col2 = st.columns([2, 1])
with col1:
    st.title("Business Scenario Simulator")
with col2:
    st.button("Reset All Parameters", type="secondary")
...
with sim_col1:
    ...
    st.markdown("Coffee Price Change (%)")
    ...
    st.markdown("Marketing Budget Change (%)")
    ...
    if st.button("Run Coffee Mate Simulation"):
...
# Add an animated coffee icon for extra wow factor
lottie_coffee = load_lottieurl("https://assets4.lottiefiles.com/packages/lf20_Cjs3wNhs2b.json")
st_lottie(lottie_coffee, height=120, key="coffee_animation")
