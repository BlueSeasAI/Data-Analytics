    ...
    try:
        ...
        if "weather" in query_lower and ("satisfaction" in query_lower or "customer" in query_lower):
            return {
                ...
                "insight": "Rainy weather correlates with 23% higher customer satisfaction but 12% lower customer counts. This indicates that while fewer customers visit during rain, those who do tend to stay longer and enjoy the cozy atmosphere more.",
                "recommendation": "Consider creating special rainy day promotions to boost attendance during wet weather, focusing on the cozy ambiance that drives higher satisfaction."
            }
            ...
            return {
                ...
                "title": "Customer Segment Behavior Analysis",
            ...
            return {
                ...
                "recommendation": "Focus on three key opportunity areas: 1) Midweek revenue enhancement, 2) Food attachment during morning coffee rush, and 3) Service optimization during peak periods."
            }
    except Exception as e:
        return {
            "type": "error",
...
if analysis_question:
    with st.spinner("AI thinking..."):  # Correct spinner usage (not st.sidebar.spinner)
        try:
    ...
    with st.spinner("AI analyzing your request..."):
        ...
        if ai_analysis["type"] == "weather_satisfaction" and 'Weather' in filtered_data.columns:
            ...
            with col2:
                ...
                <strong>AI-Generated Insight:</strong><br>{ai_analysis['insight']}
                </div>
                <div class='insight-card'>
...
tab1, tab2, tab3, tab4 = st.tabs(["Customer Segmentation", "Revenue Prediction", "Workshop Optimization", "Anomaly Detection"])
...
with tab4:
    ...
    st.info("The AI continuously monitors your business metrics for unusual patterns that might represent opportunities or problems.")
    ...
    if all(col in filtered_data.columns for col in anomaly_cols):
        ...
        fig = px.scatter(anomaly_data, x='Date', y='Revenue_Coffee', 
                         ...
                         title="AI-Detected Anomalies in Business Performance",
        ...
        if not anomalies.empty:
            ...
            for i, (_, row) in enumerate(anomalies.iterrows()):
                if i < 5:  # Limit to 5 anomalies for display clarity
                    st.markdown(f"""
                    <div class='insight-card'>
                        <strong>{row['Date'].strftime('%Y-%m-%d')} ({row['Day']})</strong><br>
                        ...
                        <strong>AI Analysis:</strong> {
                            "Unexpectedly high revenue despite lower customer count. Likely due to premium purchase behavior during the workshop event." 
...
if 'Customer_Count' in filtered_data.columns and 'Repeat_Customers' in filtered_data.columns:
    ...
    with churn_col1:
        ...
        fig.update_layout(
            title='AI-Predicted Customer Churn by Segment',
    ...
    with churn_col2:
        ...
        <div class='insight-card'>
            <strong>Recommended Intervention:</strong> The AI identified that Big Spenders who haven't visited in 14+ days and previously ordered Specialty Coffee 
            respond best to personalized offers with a 72% conversion rate. Send the "Exclusive Tasting" promotion to the 37 at-risk customers.
        </div>
        """, unsafe_allow_html=True)
        
        # Add action button
        if st.button("Generate Targeted Retention Campaign"):
            with st.spinner("AI generating campaign..."):
                time.sleep(1.2)
                st.success("Campaign generated! 37 customers targeted with personalized offers based on past purchase behavior.")
                st.download_button(
...
with ai_rec_col1:
    ...
    <div class="recommendation-card">
        ...
        <p><strong>AI Recommendation:</strong> Implement "Afternoon Productivity Bundle" with coffee + workspace discount targeting remote workers</p>
        <p><strong>Predicted Impact:</strong> +$1,270 monthly revenue</p>
        <div class="button-container">
            <button class="action-button">Implement</button>
            <button class="secondary-button">Analyze</button>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with ai_rec_col2:
    ...
    <div class="recommendation-card">
        ...
        <p><strong>AI Recommendation:</strong> Add 1 additional barista during peak hours + implement express ordering for repeat customers</p>
...
with ai_rec_col3:
    ...
    <div class="recommendation-card">
        ...
        <p><strong>AI Recommendation:</strong> Create 3-part Business Planning series with graduated pricing and partner with local business coach</p>
...
# ENHANCEMENT 5: Interactive "What-If" Scenario Simulator
st.markdown("## Business Scenario Simulator")
st.info("Use AI to predict outcomes of different business decisions")

sim_col1, sim_col2 = st.columns([1,2])

with sim_col1:
    st.subheader("Adjust Parameters")
    price_change = st.slider("Coffee Price Change (%)", -20, 30, 0, 5)
    marketing_budget = st.slider("Marketing Budget Change (%)", -50, 100, 0, 10)
...
with sim_col2:
    ...
    if 'simulation_run' in locals() and simulation_run:
        ...
        volume_modifier = 1.0
        if price_change > 0:
            volume_modifier -= price_change * 0.01  # Each 1% price increase reduces volume by 1%
        else:
            volume_modifier += abs(price_change) * 0.005  # Each 1% price decrease increases volume by 0.5%
        ...
        profit_margin = 0.3 + (0.01 * price_change if price_change > 0 else 0)  # Base 30% margin, increases with price
        new_profit = new_revenue * profit_margin
        base_profit = base_revenue * 0.3
        
        # Create metrics display
        st.subheader("AI-Predicted Business Impact")
        ...
        fig.add_trace(go.Scatter(x=months, y=forecast_rev, name="AI-Predicted Outcome", line=dict(color='#6F4E37')))
        ...
        if marketing_budget > 50 and price_change < 0:
        ...
        if loyal_program and marketing_budget < 20:
            st.info("Loyalty programs work best with adequate marketing support. Consider increasing marketing budget by at least 10% to maximize program impact.")
