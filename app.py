...
2024-01-15,Monday,510.25,225.75,175.00,0.00,82,24,58,11.11,75.00,Cold,None,None,0,0.0,20,62,278,159,193,4.2"""
    ...
    try:
        # Load directly from the string instead of from a file
        import io
        data = pd.read_csv(io.StringIO(csv_data), parse_dates=['Date'])
        st.success("Data loaded successfully from embedded string")
        
        if 'Date' in data.columns and pd.api.types.is_datetime64_dtype(data['Date']):
            return data
        
        # Create synthetic data as fallback if needed
        if 'Date' not in data.columns or len(data) == 0:
            st.warning("Creating synthetic data as fallback...")
        ...
        st.write("Creating fallback data...")
        
        # Return minimal dataset as a last resort
        return pd.DataFrame({
            'Date': pd.date_range(start='2024-01-01', periods=5),
            'Day': ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
            'Revenue_Coffee': [500, 520, 480, 510, 550],
            'Revenue_Food': [250, 230, 240, 260, 270],
            'Customer_Count': [75, 80, 72, 78, 85],
